  ...
  if((newState1 == LOW) && (oldState == HIGH)) {
    ...
    if(newState1 == LOW) {      // Yes, still low
      ...
      switch(mode) {           // Start the new animation...
        case 0:
         rainbow(0.1);
          theaterChase1(strip.Color(  255,  0,   127), 0.5);    // Black/off
          break;
        case 1:
         rainbow(0.1);
          theaterChase1(strip.Color(  255,  0, 0), 0.5); // Blue   
          
          break;
        case 2:
         rainbow(0.1);
           theaterChase1(strip.Color(  0,   255, 0), 0.5); // Blue
           
          break;
        case 3:
         rainbow(0.1);
           theaterChase1(strip.Color(  0,   0, 255),0.5); // Blue
...
void theaterChase1(uint32_t color, int wait) {
  for(int a=0; a<5; a++) {  // Repeat 10 times...
    for(int b=0; b<10; b++) { //  'b' counts from 0 to 2...
      strip.clear();         //   Set all pixels in RAM to 0 (off)
      // 'c' counts up from 'b' to end of strip in steps of 3...
      for(int c=3; c<7; c += 1) {
        strip.setPixelColor(c, color); // Set pixel 'c' to value 'color'
